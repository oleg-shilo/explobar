...
Settings: ButtonSize: 24 HistorySize: 10 ShortcutKey: 'Shift+Escape' ShowConsoleAtStartup: false
Favorites:
•	C:\Projects
...
**Examples:**
```
ShortcutKey: 'F1'               # Just F1 ShortcutKey: 'Shift+F1'         # Shift + F1 ShortcutKey: 'Ctrl+Alt+F12'     # Ctrl + Alt + F12 ShortcutKey: 'OemTilde'         # ~ key
```
...
```yaml
Items:
•	Path: 'notepad.exe' Arguments: '%f%' Tooltip: 'Open in Notepad'
``` 
...
```yaml
•	Path: 'wt.exe' Arguments: '-d %c%' Icon: 'cmd.exe' Tooltip: 'Windows Terminal here'
```

**Example: Complex command**
```yaml
Items:
•	Path: 'powershell.exe' Arguments: '-NoExit -Command "Set-Location %c%"' WorkingDir: '%c%' Icon: 'powershell.exe' Tooltip: 'PowerShell here'
```
...
- **Environment variables:** `%SystemRoot%\System32\shell32.dll,314`
...
```yaml
Items:
•	Path: 'notepad.exe' Arguments: '%f%' Shortcut: 'Ctrl+Alt+N'
```
...
```yaml
Shortcut: 'Ctrl+N'          # Ctrl + N Shortcut: 'Shift+F1'        # Shift + F1 Shortcut: 'Ctrl+Alt+F12'    # Ctrl + Alt + F12
```
...
```yaml
Items:
•	Path: 'calc.exe' Shortcut: 'Ctrl+Alt+C' Hidden: true
```
...
```yaml
Items:
•	Path: 'notepad.exe' Shortcut: 'Ctrl+Alt+N' SystemWide: true
```
...
•	Path: 'wt.exe' Arguments: '-d %c%' Shortcut: 'Ctrl+Alt+T'
Launch calculator (works anywhere)
•	Path: 'calc.exe' Shortcut: 'Ctrl+Alt+C' SystemWide: true Hidden: true
Screenshot tool (works anywhere)
•	Path: 'SnippingTool.exe' Shortcut: 'Ctrl+Alt+S' SystemWide: true Hidden: true
...
```csharp
using Explobar; using System.Windows.Forms;
public class HelloButton : CustomButton { public HelloButton() { IconIndex = 1; IconPath = "shell32.dll"; Tooltip = "Say Hello"; }
...
Load specific class
```yaml
•	Path: '{C:\Plugins\MyPlugin.dll,HelloButton}' Tooltip: 'Say hello'
```
...
```csharp
public class CustomButton : Button, ICustomButton { // Set in constructor public int IconIndex { get; protected set; } public string IconPath { get; protected set; } public string Tooltip { get; protected set; } public bool IsExpandabe { get; protected set; }  // Show dropdown indicator
// Override these
...
```csharp
public override void OnClick(ClickArgs args) { // Current folder string folder = args.Context.RootPath;
// Selected files
...
```csharp
using Explobar; using System.IO; using System.Linq; using System.Text; using System.Windows.Forms;
public class FolderLister : CustomButton { public FolderLister() { IconIndex = 4; IconPath = "shell32.dll"; Tooltip = "Copy folder listing"; }
public override void OnClick(ClickArgs args)
{
    try
    {
...
```csharp
using Explobar; using System.IO; using System.Linq; using System.Windows.Forms;
public class FileCounter : CustomButton { public FileCounter() { IconIndex = 71; IconPath = "shell32.dll"; Tooltip = "Count files and folders"; }
public override void OnClick(ClickArgs args)
{
...
```csharp
using Explobar; using System.IO; using System.Linq; using System.Windows.Forms;
public class ToolsMenu : CustomButton { public ToolsMenu() { IconIndex = 137; IconPath = "shell32.dll"; Tooltip = "Tools"; IsExpandabe = true;  // Shows dropdown indicator }
public override void OnClick(ClickArgs args)
{
...
// Show context menu and keep toolbar open CustomButton.PopupMenu(this, args, () => menu);
// Navigate Explorer to folder (Ctrl+click opens in new tab) CustomButton.NavigateToPath(args.Context, @"C:\Windows");
// Get fresh Explorer context (avoids COM RCW errors) var fresh = args.Context.GetFreshCopy();
...
```csharp
public override void OnClick(ClickArgs args) { try { // Your code } catch (Exception ex) { MessageBox.Show($"Error: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); } }
```

**2. Validate input**
```csharp
if (string.IsNullOrEmpty(args.Context.RootPath)) { MessageBox.Show("No folder selected"); return; }
if (!args.Context.SelectedItems.Any()) { MessageBox.Show("No files selected"); return; }
```
...
public override void OnClick(ClickArgs args) { args.DoNotHideToolbar = true;
Task.Run(async () =>
{
    // Long operation
...
// Avoid "COM object separated from RCW" errors var fresh = args.Context.GetFreshCopy(); Explorer.SelectItem(fresh.Window, filePath);
...
```c#
Console.WriteLine($"Processing: {args.Context.RootPath}"); Runtime.Log("Custom log message");
```
...
```yaml
Items:
•	Path: '{C:\Plugins\MyTools.dll}' Tooltip: 'First button found'
```


**Load specific class:**

•	Path: '{C:\Plugins\MyTools.dll,FolderLister}' Tooltip: 'List folder contents'
...
Items:
•	Path: '{C:\Plugins\MyTools.dll,FolderLister}' Icon: 'shell32.dll,100' Tooltip: 'Custom tooltip'

**With keyboard shortcut:**

```yaml
Items:
•	Path: '{C:\Plugins\MyTools.dll,QuickAction}' Shortcut: 'Ctrl+Alt+Q' Hidden: true
```
Explobar Configuration
Settings: ButtonSize: 24 HistorySize: 15 ShortcutKey: 'Shift+Escape' ShowConsoleAtStartup: false
Favorites:
•	C:\Projects
•	%UserProfile%\Downloads
•	%UserProfile%\Documents
...
•	Path: '{separator}'
Custom commands
•	Path: 'wt.exe' Arguments: '-d %c%' Icon: 'cmd.exe' Tooltip: 'Windows Terminal' Shortcut: 'Ctrl+Alt+T'
•	Path: 'notepad.exe' Arguments: '%f%' Tooltip: 'Edit in Notepad' Shortcut: 'Ctrl+Alt+N'
•	Path: 'explorer.exe' Arguments: '/select,%f%' Tooltip: 'Show in Explorer'
•	Path: '{separator}'
Plugins
•	Path: '{C:\Plugins\ExplobarTools.dll,FolderLister}' Tooltip: 'Copy folder listing'
•	Path: '{C:\Plugins\ExplobarTools.dll,FileCounter}' Shortcut: 'Ctrl+Alt+I'
•	Path: '{separator}'
